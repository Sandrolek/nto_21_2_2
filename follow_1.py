...
def image_callback(data):
    ...
    if dir_state["top"] and not dir_state["left"] and not dir_state["right"]: # only forward
        ...
        if rect_right[0][1] < HEIGHT // 2:
            ...
            rospy.sleep(2)
            print("rotated")
            navigate(x=telem.x, y=telem.y, z=1, frame_id="aruco_map", yaw=telem.yaw - 1.6, yaw_rate=0, auto_arm=False)
            rospy.sleep(2)
            print("upped")
            navigate(x=1.3, y=0, z=0, frame_id="body", yaw=0, yaw_rate=0, auto_arm=False)
            rospy.sleep(6)
            print("translated")
            turn_state = True
            start_turn_time = time.time()
        ...
        if rect_left[0][1] < HEIGHT // 2:
            ...
            rospy.sleep(2)
            print("rotated")
            navigate(x=telem.x, y=telem.y, z=1, frame_id="aruco_map", yaw=telem.yaw + 1.57, yaw_rate=0, auto_arm=False)
            rospy.sleep(2)
            print("upped")
            navigate(x=1.3, y=0, z=0, frame_id="body", yaw=0, yaw_rate=0, auto_arm=False)
            rospy.sleep(6)
